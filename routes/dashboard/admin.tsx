...
function AdminPage({ state }) {
  return (
    <main class="min-h-screen bg-base-100 text-base-content p-8">
      ...
      <p class="mt-2">
        Hola{" "}
        {state.auth.user?.displayName}. Esta vista es exclusiva para rol admin.
      </p>
      <a href="/admin/users" class="inline-block mt-4 text-primary underline">
...
export const handler = define.handlers({
  GET(ctx) {
    ...
    return ctx.render(
      <AdminPage state={ctx.state} url={ctx.url} params={ctx.params} />,
    );
  },
});
