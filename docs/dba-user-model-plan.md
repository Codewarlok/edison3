...
## Objetivo
Definir un modelo de datos simple y trazable para `users`, `sessions` y `audit_events` en DenoKV, con:
...
> Nota: DenoKV no tiene "índices secundarios" como SQL; se modelan como claves auxiliares.
...
### Índices
- `['sessions_by_token_hash', tokenHash] -> sessionId` (lookup en middleware auth)
- `['sessions_by_user', userId, sessionId] -> expiresAt` (listar/revocar por usuario)
...
- PocketBase actual tiene usuarios auth con `id`, `email`, `name`, `verified`, `created`, `updated`, estado/flags.
...
2. Password strategy:
   - Opción segura recomendada: forzar reset de contraseña en primer login post-migración.
   - Alternativa: migración perezosa si algoritmo PB es verificable (solo si se implementa verificador confiable).
...
- **Colisión/case en email** → normalización estricta + índice único por email normalizado.
...
Con esto se cubre base sólida para auth ELLIS en DenoKV, RBAC inicial y trazabilidad operativa.